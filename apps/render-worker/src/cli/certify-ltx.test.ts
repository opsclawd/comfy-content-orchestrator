...
import type { CertificationEnvironment, LtxCertificationArtifact } from "@cco/contracts";
...
describe("certify:ltx CLI", () => {
  ...
  const mockPassedArtifact: LtxCertificationArtifact = {
    version: 1,
    runId: "run-001",
  ...
  const mockFailedArtifact: LtxCertificationArtifact = Object.freeze({
    ...mockPassedArtifact,
    status: "failed",
  ...
  const createStandardDependencies = (
    overrides?: Partial<CertifyLtxCliDependencies>
  ): CertifyLtxCliDependencies => ({
    loadCertificationProfile: vi.fn().mockResolvedValue(mockProfile),
  ...
  describe("Behavioral Invariants", () => {
    it("completes all preflight checks before starting telemetry or rendering (preflight-precedes-side-effects)", async () => {
      ...
      // Verify all preflight checks happen before sampler.start
      ...
      for (const phase of preflightPhases) {
        expect(order).toContain(phase);
      }

      // Also verify refusal stops execution before any side effects
      const refusedDeps = createStandardDependencies({
        verifyGoldMasterProvenance: vi.fn().mockImplementation(() => {
          throw new PreflightError("Hash mismatch drift detected");
        }),
        createTelemetrySampler: vi.fn().mockReturnValue(samplerControl),
        createRenderEngine: vi.fn().mockReturnValue(renderEngine),
        runCertification: vi.fn(),
        writeCertificationArtifacts: vi.fn()
      });

      const refusedIo = createMockIo();
      const refusedExitCode = await runCertificationCli(standardArgs, refusedIo, refusedDeps);

      expect(refusedExitCode).toBe(1);
      expect(samplerControl.start).toHaveBeenCalledTimes(0);
      expect(renderEngine.queueRender).toHaveBeenCalledTimes(0);
      expect(refusedDeps.runCertification).not.toHaveBeenCalled();
      expect(refusedDeps.writeCertificationArtifacts).not.toHaveBeenCalled();
    });
    ...
    it("does not execute the CLI when imported (direct-entry-is-testable)", async () => {
      ...
      const mod = await import("./certify-ltx.js");
      expect(mod.parseCertifyLtxCliArgs).toBeDefined();
  ...
  describe("CLI Flag Parsing", () => {
    it("parses all required and optional flags correctly", () => {
      const parsed = parseCertifyLtxCliArgs([
        "--comfyui-dir",
        "/custom/comfyui",
        "--comfyui-url",
      ...
      expect(parsed).toEqual({
        kind: "run",
        options: {
          comfyUiDir: "/custom/comfyui",
          comfyUiUrl: "http://192.168.1.50:8188",
          comfyUiPid: 4567,
          goldMasterProvenancePath: "/custom/gold.json",
    ...
    it("returns help for --help and -h", () => {
      expect(parseCertifyLtxCliArgs(["--help"])).toEqual({ kind: "help" });
      expect(parseCertifyLtxCliArgs(["-h"])).toEqual({ kind: "help" });
      expect(parseCertifyLtxCliArgs(["--comfyui-dir", "/foo", "--help"])).toEqual({ kind: "help" });
    });
    ...
    it("rejects invalid comfyui-pid values", () => {
      ...
      expect(() => parseCertifyLtxCliArgs([...base, "--comfyui-pid", "abc"])).toThrow(
        /positive integer/
      );
      expect(() => parseCertifyLtxCliArgs([...base, "--comfyui-pid", "0"])).toThrow(
        /positive integer/
      );
      expect(() => parseCertifyLtxCliArgs([...base, "--comfyui-pid", "-5"])).toThrow(
        /positive integer/
      );
      expect(() => parseCertifyLtxCliArgs([...base, "--comfyui-pid", "1.5"])).toThrow(
        /positive integer/
      );
    });
    ...
    it("rejects invalid gpu-index values", () => {
      ...
      expect(() => parseCertifyLtxCliArgs([...base, "--gpu-index", "-1"])).toThrow(
        /non-negative integer/
      );
      expect(() => parseCertifyLtxCliArgs([...base, "--gpu-index", "xyz"])).toThrow(
        /non-negative integer/
      );
      expect(() => parseCertifyLtxCliArgs([...base, "--gpu-index", "2.3"])).toThrow(
        /non-negative integer/
      );
    });
    ...
    it("rejects invalid run-id values", () => {
      ...
      expect(() => parseCertifyLtxCliArgs([...base, "--run-id", "INVALID_UPPER"])).toThrow(
        /run-id/i
      );
      expect(() => parseCertifyLtxCliArgs([...base, "--run-id", "../traversal"])).toThrow(
        /run-id/i
      );
      expect(() => parseCertifyLtxCliArgs([...base, "--run-id", "slash/id"])).toThrow(/run-id/i);
      expect(() => parseCertifyLtxCliArgs([...base, "--run-id", "-bad-start"])).toThrow(/run-id/i);
      expect(() => parseCertifyLtxCliArgs([...base, "--run-id", ""])).toThrow(/run-id/i);
    });

    it("rejects duplicate flags", () => {
      expect(() =>
        parseCertifyLtxCliArgs([...standardArgs, "--gpu-index", "0", "--gpu-index", "1"])
      ).toThrow(/duplicate flag/i);
    });

    it("rejects unknown flags", () => {
      expect(() => parseCertifyLtxCliArgs([...standardArgs, "--unrecognized-flag"])).toThrow(
        /unknown flag/i
      );
    });

    it("rejects positional arguments", () => {
      expect(() => parseCertifyLtxCliArgs([...standardArgs, "extra-positional-arg"])).toThrow(
        /unexpected argument/i
      );
    });

    it("rejects flags missing values", () => {
      expect(() => parseCertifyLtxCliArgs(["--comfyui-dir", "/comfy", "--comfyui-url"])).toThrow(
        /requires a value/i
      );
    });
  });
  ...
  describe("Package scripts and configuration", () => {
    it("render-worker package.json exposes certify:ltx with tsx entrypoint", async () => {
      ...
      expect(parsed.scripts["certify:ltx"]).toBe("tsx src/cli/certify-ltx.ts");
    });

    it("root package.json exposes certify:ltx filter command", async () => {
