...
sole resolver wired into review-action endpoints and reads `tailscale-user-login`
/ `tailscale-user-name` HTTP headers that only Tailscale itself can set, requires
a `trustedProxies` allowlist (no trust-by-default), and fails closed with
`ReviewerIdentityUnavailableError` when identity cannot be resolved. The
fallback-identity mode for dev/test is explicitly mutually exclusive with the
trusted-proxies mode (enforced at construction), so there is no possibility of a
phantom audit identity from a misconfigured server.
