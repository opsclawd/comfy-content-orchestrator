...
- **One-Way Profile Dispatch:** Generation dispatch evaluates candidate selection to determine the execution profile. For reviewed visual scenes with an approved candidate, production dispatch unconditionally selects the certified conditioned profile (`LTX_25_720P_5S_I2V_V1` / `ltx-25-720p-97f-i2v`) without override toggles or opt-outs.
- **Control-Plane Selection & `workflowTemplate` Worker Resolution:** The control plane (`EnqueueSceneProductionRenderUseCase`) populates `job.workflowTemplate`. Render workers resolve the certified profile, injection topology, and execution flow using `job.workflowTemplate` (`ltx-25-720p-97f-i2v` routes to the I2V conditioning graph: Node 20 `LoadImage`, Node 21 `ImageScale` with Lanczos 1280x720 center crop, Node 22 `LTXVImgToVideo`).
- **Preservation of Legacy Profile for Non-Reviewed Paths:** The legacy text-to-video profile (`LTX_25_720P_5S_V1` / `ltx-25-720p-97f`) is retained in the platform strictly for separate legacy or unreviewed execution paths, never as an escape hatch or fallback for reviewed visual production.
...
- [x] **External Processing Policy Gate:** disabling each cloud-processing flag prevents prohibited provider calls and produces documented degraded/manual behavior. *Verified 2026-09-07 (issue #165): all three cloud-processing flags (`allowCloudPlanning`, `allowCloudVisualQA`, and `allowCloudVoice`) are enforced and covered by tests proving provider calls do not happen when disabled, consolidated in `packages/application/src/use-cases/external-processing-policy-gate.test.ts` (13/13 passed) alongside focused unit suites. All documented PRD §3.3 degraded/manual behaviors are verified: `allowCloudPlanning=false` requires creator-authored SceneConfiguration and rejects brief submission (`planning-orchestration-kernel.ts`, `submit-scene-creation.ts`); `allowCloudVisualQA=false` presents candidates unranked for human review and skips image resolution (`candidate-ranking-orchestrator.ts`, `rank-review-candidates.ts`); `allowCloudVoice=false` prohibits cloud voice synthesis and accepts human/local audio upload into immutable storage with `source: { kind: "uploaded" }` (`synthesize-voiceover.ts`, `voice-synthesis-port.ts`, `voice-synthesis-factory.ts`).*
