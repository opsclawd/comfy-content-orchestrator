...
- **One-Way Profile Dispatch:** Generation dispatch evaluates candidate selection to determine the execution profile. For reviewed scenes with an approved candidate, normal reviewed production dispatches using the certified conditioned profile (`LTX_25_720P_5S_I2V_V1` / `ltx-25-720p-97f-i2v`) by default.
...
- **Legacy Opt-Out Control:** The options `forceLegacyTextProfile: true` or `enableConditionedProfile: false` allow callers to explicitly route through the legacy text-only pipeline if necessary, while default production unconditionally selects conditioned I2V.
