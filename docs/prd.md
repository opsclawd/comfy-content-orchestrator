...
- [ ] **Review Hub Browser Access Gate:** the Creative Director completes a real review action end to end from the remote review plane. *Updated 2026-09-03: TLS and reviewer authentication are now resolved by decision (deferred, not built — see issue #90, closed "not planned," and issue #152). The only remaining blocker is a real end-to-end review action from the actual Creative Director device in Ottawa, which additionally requires a real campaign/scene to review (see issue #154, #155) — no agent can execute this gate; per issue #68's own comments, it needs a human at that device.*
