...
import {
  ...
  canonicalizeOrchestrationRequest,
  computeCampaignRequestHash,
  computeOrchestrationRequestHash,
  type CampaignRequestCanonicalizationInput,
...
describe("Campaign and Scene Creation Contracts", () => {
  ...
  describe("canonicalizeCampaignRequest and computeCampaignRequestHash", () => {
    const baseInput: CampaignRequestCanonicalizationInput = {
      clientId: "018e69e0-8a6a-72cb-b1b7-ec79a1f73801",
      title: "Summer 2026 Collection",
      targetPlatform: "tiktok",
      targetTotalDurationMs: 15000,
    ...
    it("canonical string is independent of object key insertion order", async () => {
      const inputReordered: CampaignRequestCanonicalizationInput = {
        targetTotalDurationMs: 15000,
        title: "Summer 2026 Collection",
        sceneCountOverride: undefined,
        targetPlatform: "tiktok",
    ...
    it("produces different hash when sceneCountOverride is defined vs undefined (Auto-mode)", async () => {
    ...
    it("preserves identical hash without totalScenes substitution on Auto mode retry", async () => {
      const initialRequest: CampaignRequestCanonicalizationInput = {
        clientId: "018e69e0-8a6a-72cb-b1b7-ec79a1f73801",
      ...
      const retryRequest: CampaignRequestCanonicalizationInput = {
        clientId: "018e69e0-8a6a-72cb-b1b7-ec79a1f73801",
        title: "Auto Mode Campaign",
        targetTotalDurationMs: 15000,
        sceneCountOverride: undefined
      ...
      const flawedSubstitutedRequest: CampaignRequestCanonicalizationInput = {
        clientId: "018e69e0-8a6a-72cb-b1b7-ec79a1f73801",
        title: "Auto Mode Campaign",
        targetTotalDurationMs: 15000,
        sceneCountOverride: 3
    ...
    it("is sensitive to changes in each field", async () => {
      const hashBase = await computeCampaignRequestHash(baseInput);
    ...
    it("is sensitive to changes in creative brief and handles deep key reordering", async () => {
      const briefA = {
      ...
      const brief1 = { description: "Promo", visualStyle: "cinematic", title: "Title A" };
      const brief2 = { title: "Title A", visualStyle: "cinematic", description: "Promo" };
      const hashBrief1 = await computeCampaignRequestHash({ ...baseInput, brief: brief1 });
      const hashBrief2 = await computeCampaignRequestHash({ ...baseInput, brief: brief2 });
      expect(hashBrief1).toBe(hashBrief2);
    });
    ...
    it("matches the pinned canonical JSON representation fixture", () => {
      const fixture: CampaignRequestCanonicalizationInput = {
    ...
    it("provides alias exports matching canonical functions", async () => {
      expect(canonicalizeOrchestrationRequest).toBe(canonicalizeCampaignRequest);
      expect(computeOrchestrationRequestHash).toBe(computeCampaignRequestHash);
    });
  });
});
